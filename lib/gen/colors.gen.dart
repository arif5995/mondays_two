/// GENERATED CODE - DO NOT MODIFY BY HAND
/// *****************************************************
///  FlutterGen
/// *****************************************************

// coverage:ignore-file
// ignore_for_file: type=lint
// ignore_for_file: directives_ordering,unnecessary_import

import 'package:flutter/painting.dart';
import 'package:flutter/material.dart';

class ColorName {
  ColorName._();

  /// Color: #e9d8f2
  static const Color backgroundPurple = Color(0xFFE9D8F2);

  /// Color: #D6FEAF
  static const Color greenLow = Color(0xFFD6FEAF);

  /// Color: #21FA1C
  static const Color greenMedium = Color(0xFF21FA1C);

  /// Color: #C4C4C4
  static const Color iconGrey = Color(0xFFC4C4C4);

  /// Color: #6a0dad
  static const Color iconPurple = Color(0xFF6A0DAD);

  /// Color: #DADADA
  static const Color iconWhite = Color(0xFFDADADA);

  /// Color: #F9A26C
  static const Color lightOrange = Color(0xFFF9A26C);

  /// Color: #F26627
  static const Color orange = Color(0xFFF26627);

  /// Color: #a167c9
  static const Color purpleTransparant = Color(0xFFA167C9);

  /// Color: #FEAFAF
  static const Color redLow = Color(0xFFFEAFAF);

  /// Color: #FA1C1C
  static const Color redMedium = Color(0xFFFA1C1C);

  /// Color: #303030
  static const Color textBlack = Color(0xFF303030);

  /// Color: #325D79
  static const Color textDarkBlue = Color(0xFF325D79);

  /// Color: #F4F4F4
  static const Color textFieldBackgroundGrey = Color(0xFFF4F4F4);

  /// Color: #BBBBBB
  static const Color textFieldHintGrey = Color(0xFFBBBBBB);

  /// Color: #7B7B7B
  static const Color textGrey = Color(0xFF7B7B7B);

  /// Color: #AAAAAA
  static const Color textLightGrey = Color(0xFFAAAAAA);

  /// Color: #FFFFFF
  static const Color white = Color(0xFFFFFFFF);

  /// Color: #E5E5E5
  static const Color whiteBackground = Color(0xFFE5E5E5);
}
